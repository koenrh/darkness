...
on run argv
  -- darkness start, e.g. 64800for 18:00
  -- darkness end, e.g. 25200 for 7:00
  ...
  if not dark_mode_enabled and (time of (current date) < darknessEnd or time of (current date) > darknessStart) then
    set_darkness(true)
